...
| **What customer segments exist?** | K-Means (Optimized) | Silhouette = 0.614 | 4 actionable customer segments |
...
**Algorithm**: K-Means with k=4 clusters on MinMax-scaled Income + TotalSpend  
**Validation**: Silhouette Score = 0.614 (improved from 0.165 with feature selection + scaling optimization)

| Segment | Size | Avg Income | Avg Spend | Response Rate | Key Characteristics |
|---------|------|------------|-----------|---------------|---------------------|
| **💎 Premium Champions** | 26% | $72,887 | $1,391 | 28% | High income, high spenders, most responsive |
| **📈 Digital Deal Hunters** | 24% | $59,610 | $792 | 13% | Above-avg income, deal seekers, digital-native |
| **📉 Empty Nesters** | 25% | $42,537 | $128 | 6% | Moderate income, low engagement |
| **🔻 Budget Conscious** | 25% | $29,748 | $102 | 13% | Lower income, young families |
...
```
models/
├── best_regressor.joblib          # Random Forest Regressor (run 02_regression.ipynb)
├── best_classifier.joblib         # Random Forest Classifier (run 03_classification.ipynb)
├── kmeans_model.joblib            # K-Means Clustering (run 04_clustering.ipynb)
├── mlp_regressor.keras            # Deep Learning Regressor (run 05_deep_learning.ipynb)
├── mlp_classifier.keras           # Deep Learning Classifier (run 05_deep_learning.ipynb)
...
> **Note**: Model files are generated by running the corresponding notebooks. Execute notebooks in order (01 → 05) to generate all artifacts.
...
├── 02b_regression_new_customers.ipynb  # Cold Start Model
